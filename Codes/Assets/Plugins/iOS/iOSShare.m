...
+(id) withTitle:(char*)title withMessage:(char*)message{
    ...
    return [[iOSShare alloc] initWithTitle:title withMessage:message];
}
...
+(id) withText:(char*)text withURL:(char*)url withImage:(char*)image withSubject:(char*)subject{
    ...
    return [[iOSShare alloc] initWithText:text withURL:url withImage:image withSubject:subject];
}
...
-(id) initWithText:(char*)text withURL:(char*)url withImage:(char*)image withSubject:(char*)subject{
    ...
    if(mImage != NULL && mImage.length > 0){
        ...
        if([mImage hasPrefix:@"http"])
        {
            ...
            NSData* imageBase64Data = [[NSData alloc]initWithBase64EncodedString:mImage options:NSDataBase64DecodingIgnoreUnknownCharacters];
...
-(_Bool) isStringValideBase64:(NSString*)string{
...
void showAlertMessage(struct ConfigStruct *confStruct) {
    instance = [iOSShare withTitle:confStruct->title withMessage:confStruct->message];
}

void showSocialSharing(struct SocialSharingStruct *confStruct) {
    instance = [iOSShare withText:confStruct->text withURL:confStruct->url withImage:confStruct->image withSubject:confStruct->subject];
}
